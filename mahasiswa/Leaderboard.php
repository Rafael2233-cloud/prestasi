...
if ($filter_tahun !== '') {
    $base_query = str_replace("LEFT JOIN prestasi p ON m.id = p.mahasiswa_id", "LEFT JOIN prestasi p ON m.id = p.mahasiswa_id AND p.tahun = '" . mysqli_real_escape_string($conn, $filter_tahun) . "'", $base_query);
}

$where_clauses = ["1=1"];
if ($filter_prodi !== '') {
    $where_clauses[] = "m.prodi = '" . mysqli_real_escape_string($conn, $filter_prodi) . "'";
}
if ($filter_angkatan !== '') {
    $where_clauses[] = "m.angkatan = '" . mysqli_real_escape_string($conn, $filter_angkatan) . "'";
}
...
<style>
    ...
    .top-card::before { content: ''; position: absolute; inset: 0; background-image: url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" fill="none" stroke="%23e2e8f0" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M30 80 C 10 70, 10 30, 30 20" /><path d="M70 80 C 90 70, 90 30, 70 20" /><path d="M30 80 L 70 80" /><circle cx="50" cy="50" r="15" fill="%23f8fafc" stroke="none"/></svg>'); background-size: 120%; background-position: center 20%; background-repeat: no-repeat; opacity: 0.6; z-index: 0; }
    ...
    .page-btn:hover:not(.active):not(.dots) { background: #f1f5f9; text-decoration: none; }
    .page-btn.dots { border: none; cursor: default; }
    @media (max-width: 1024px) { .top3-grid { grid-template-columns: 1fr; } }
</style>
    ...
    <form action="Leaderboard.php" method="GET" id="filterForm">
        <div class="filter-card">
            ...
            <div class="filter-controls">
                ...
                <a href="Leaderboard.php" class="btn-reset">
            ...
            <table class="custom-table" id="leaderboardTable">
                ...
                <tbody id="tableBody">
                    ...
                    foreach($leaderboard_data as $row): 
                        ...
                        // Highlight current student if applicable (from $_SESSION['mahasiswa_id'])
                        $is_current = isset($mahasiswa_id) && $row['id'] == $mahasiswa_id;
                        $highlight_bg = $is_current ? 'background-color: #eff6ff;' : '';
                    ?>
                    <tr class="table-row item-anim <?= $d_class ?>" style="<?= $highlight_bg ?>">
                        <td><?= $rank_html ?></td>
                        <td><?= htmlspecialchars($row['nim']) ?></td>
                        <td class="col-name">
                            <?= htmlspecialchars($row['nama']) ?>
                            <?php if($is_current): ?>
                                <span style="background: #2563eb; color: white; font-size: 10px; padding: 2px 6px; border-radius: 4px; margin-left: 8px;">Anda</span>
                            <?php endif; ?>
                        </td>
