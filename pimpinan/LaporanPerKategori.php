...
$query = "
    ...
    WHERE p.status = 'approved' AND p.kategori IS NOT NULL AND p.kategori != ''
";

if ($search !== '') {
    $query .= " AND p.kategori LIKE '%" . $conn->real_escape_string($search) . "%'";
}

$query .= " GROUP BY p.kategori ORDER BY total_poin DESC, p.kategori ASC";
...
<head>
    ...
    <title>Laporan per Kategori - SIPRESMA</title>
...
<body class="dashboard-body new-dashboard">
    ...
    <main class="main-content-new" id="mainContent">
        ...
        <div class="content-wrapper-new">
                ...
                <div class="page-header-flex">
                    <div class="page-title-new">
                        <div class="page-header-text">
                            <h1 style="margin: 0 0 5px 0; font-size: 24px; font-weight: 800; color: #0f172a;">Laporan per Kategori</h1>
                            <p style="font-family: 'Inter', sans-serif; font-size: 13px; font-weight: 400; color: #64748b; line-height: 1.5; margin: 0;">Rekapitulasi prestasi mahasiswa dikelompokkan berdasarkan kategori prestasi.</p>
                        </div>
                    </div>
                    <div style="display: flex; flex-direction: column; align-items: flex-end; gap: 15px;">
                        <div class="breadcrumb-top" style="margin-bottom: 0; font-size: 13px; color: #64748b;">
                            <a href="Dashboard.php" style="color: #2563eb; text-decoration: none;">Beranda</a> <span>/</span> Laporan per Kategori
                        </div>
                    </div>
                </div>
            </div>
            ...
            <!-- Filter Section -->
            <form action="LaporanPerKategori.php" method="GET" id="filterForm">
                <div class="filter-card">
                    ...
                    <div class="filter-controls">
                        <div class="filter-group">
                            <label class="filter-label">Cari Kategori</label>
                            <input type="text" class="filter-input" name="search" value="<?= htmlspecialchars($search) ?>" placeholder="Masukkan nama kategori...">
                        </div>
                        <button type="submit" class="btn-search"><i class="fa-solid fa-magnifying-glass"></i> Cari</button>
                        <a href="LaporanPerKategori.php" class="btn-reset"><i class="fa-solid fa-rotate-right"></i> Reset</a>
                    </div>
                </div>
            </form>
                ...
                <div class="table-responsive">
                    <table class="custom-table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td><?= $no++ ?></td>
                                <td class="col-highlight"><i class="fa-solid fa-layer-group" style="color: #94a3b8; margin-right: 8px;"></i> <?= htmlspecialchars($row['kategori']) ?></td>
